...
public class ScreenShotTakesByAmazon {
   ...
   public static void SearchBox(String ProductName,String ProCategories) throws IOException {
	   WebElement SearchText,Ologo, Categories,Botton;
	   SearchText=driver.findElement(By.xpath("//input[@id='twotabsearchtextbox']"));
	   SearchText.sendKeys(ProductName);
	   
	   //TAKE SCREENSHOT FOR WEBELEMENTS
	   Ologo=driver.findElement(By.id("nav-logo-sprites"));
	   ScreenShotMethodsInSelenium.takeScreenShotWebElements(Ologo, "Logo_image");
	   ...
	   ScreenShotMethodsInSelenium.takeScreenShotFile(driver, "Home_image");//TAKE SCREENSHOT
	   
	   Botton=driver.findElement(By.xpath("//input[@id='nav-search-submit-button']"));
	   Botton.click();
	   ScreenShotMethodsInSelenium.takeScreenShotByte(driver, "button_click");//TAKE SCREENSHOT
   }
   public static void PageFoundResult() throws IOException {
	  ...
	  // System.out.println("Extracted Number: " + Result);
	   if (int1>0) {
		...
		ScreenShotMethodsInSelenium.takeScreenShotBase64(driver, "Result_image");//TAKE SCREENSHOT
		//SearchResult();
	} else {
